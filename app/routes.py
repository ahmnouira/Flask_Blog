...
posts_fake = [  # create a fake posts : list of dict
    {"author": {"username": "Alex"}, "body": "This is a good day"},
    {"author": {"username": "Myriam"}, "body": "Wow you are amazing!"},
    {"author": {"username": "Ahmed"}, "body": "Can I plat this game"},
]
attempts = 0
...
)  # decorator : modifies the function that flow it
...
def index():
    ...
    next_url = (
        url_for("index", page=all_posts.next_num) if all_posts.has_next else None
    )  # check for next page
    prev_url = (
        url_for("index", page=all_posts.prev_num) if all_posts.has_prev else None
...
)  # this view function accepts GET , POST methods
def login():
    user_ = None
    global attempts
    ...
    if form_.validate_on_submit():  # when validate the from
        ...
        if user_ is None or not user_.check_password(
            form_.password.data
        ):  # no user input  or wrong password
...
def user(username):
    user_porfile = User.query.filter_by(
        username=username
    ).first_or_404()  # the first ( = exist 'username' ) or
    # 404 to the client
    ...
    my_posts = user_porfile.posts.order_by(Post.timestamp.desc()).paginate(
        page, App.config["POSTS_PER_PAGE"], False
    )

    # check for next page
    next_url = (
        url_for("user", username=user_porfile.username, page=my_posts.next_num)
        if my_posts.has_next
        else None
    )
    # ckeck for previous page
    prev_url = (
        url_for("user", username=user_porfile.username, page=my_posts.prev_num)
...
def sendEmail(username):
    ...
    if email_form.validate_on_submit():
        ...
        send_email(
            ...
            recipients=[user_porfile.email],
            text_body="",
            html_body=body
            + """<br><h2> To reply go to <a href="https://ahmed-nouira-blog4.herokuapp.com"> The BLOG ^^ </a></h2>
                   """,
        )
...
def edit_profile():  # !!!!!!!!!
    ...
    if form_edit.validate_on_submit():
        ...
        new_user = User(
            username=form_edit.username.data, about_me=form_edit.username.data
        )
    ...
    return render_template("edit_profile.html", title="Edit Profile", form=form_edit)
...
def reset_password_request():
    ...
    ):  # making sure the user is not logged in( have right password and email )
        return redirect(url_for("index"))
    ...
    return render_template(
        "reset_password_request.html", title="Reset_Request", form=form_rest
    )
